...
/** Laser-print artwork uploaded by the customer at checkout (own user folder in storage). */
export async function uploadOrderLaserImageFromUri(params: {
  localUri: string;
  userId: string;
}): Promise<string> {
  ...
  const path = `${userId}/laser-${randomSuffix()}.${ext}`;
